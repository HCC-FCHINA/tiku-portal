...
import com.chaofan.modules.tiku.model.*;
import com.chaofan.modules.tiku.service.*;
...
public class SubjectController {
    ...
    public ModelAndView indexAction(@PathVariable String subjectId , HttpServletRequest request , ModelAndView model) {
		...
		// 获取第一本书
		List<KnowPointEntity> bookList = iKnowPointService.selectList(Condition.create().eq("course_id", course.getId()));
		List<ChapterEntity> chapterList = null;
		if(bookList.size() > 0) {
			KnowPointEntity book = bookList.get(0);
			chapterList = iChapterService.selectList(Condition.create().eq("book_id", book.getId()).eq("tree_level", "1"));
		}
		// 获取最新的试卷
