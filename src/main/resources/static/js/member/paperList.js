...
var vm = new Vue({
	...
	methods: {
		reload: function (currPage) {
			vm.loading = true;
			axios({method: 'get',url: mainHttp + "member/paper/list.html?page="+currPage+"&limit=6"}).then(function(response){
		...
		queryAreaList:function(){
			axios.get(mainHttp + 'area/findProvince.html').then(function(response){
